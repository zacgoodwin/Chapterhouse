...
describe TlcCharacter::BaseBuilder do
  subject(:build) { described_class.new.call(result: result) }

  let(:result) { { main_class: 'paladin', species: 'human' } }

  it 'starts TLC characters at level 3', :aggregate_failures do
    expect(described_class::START_LEVEL).to eq(3)
    expect(build[:level]).to eq(3)
    expect(build[:classes]).to eq('paladin' => 3)
    expect(build[:subclasses]).to eq('paladin' => nil)
  end
  ...
  describe 'creation guide' do
    ...
    context 'when the guide is skipped' do
      let(:result) { { main_class: 'paladin', skip_guide: true } }
