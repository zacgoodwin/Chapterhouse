...
const { default: dnd2024Config } = await import('../../app/javascript/applications/CharKeeperApp/data/dnd2024.json');
...
const renderForm = (locale = 'en', onCreateCharacter = async () => null) => {
  ...
  const html = renderToString(() => Dnd2024CharacterForm({
    ...
    setCurrentTab: () => {},
    homebrews: () => undefined,
    dnd2024Races: () => dnd2024Config.species
  }));
...
test('a dirtied, unsaved form does not leak into the next mount', () => {
  // Dnd2024CharacterForm handed DND2024_DEFAULT_FORM straight to createStore: every
  ...
  speciesSelect(abandoned).onSelect('dwarf');
