...
(c) Feats/features/spells/items are DB rows seeded by `db/seeds.rb` from `db/data/<sys>/*.{json,csv}` (upsert_all with `type` set). Feat record: `{slug, title{en}, description, origin, origin_value, kind, modifiers, eval_variables}` (English-only since v0.5.0.0). The `feats` table (schema L235) is the richest content store.
